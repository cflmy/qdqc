...
文章页接近论文阅读栏（发丝线标题区、首字下沉、章节编号、宽边距）。
动效由 styles/brand-motion.mq.md 与 public/client.mq.md（WASM）协作完成。禁止在本文件顶层使用无序列表符号。
...
覆盖 Marqdo 内置壳在小屏上的双列默认（`壳样式=minimal` 后冲突已减轻；此处仍保证杂志抽屉布局）。
